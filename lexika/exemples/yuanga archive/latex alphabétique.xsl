...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0" xmlns:exslt="http://exslt.org/common" xmlns:regexp="http://exslt.org/regular-expressions">
    ...
    <xsl:template match="Élément/Élément">
        ...
        <xsl:variable name="bloc_entrées">
            <xsl:apply-templates select="/RessourceLexicale/Dictionnaire/EntréeLexicale[regexp:match(translate(Lemme/ReprésentationDeForme, '-', ''), $expression_rationnelle_graphèmes)]"/>
        </xsl:variable>
    ...
    <!--<xsl:template match="Dictionnaire">-->
        <!--<xsl:for-each select="EntréeLexicale">-->
            <!--<xsl:variable name="caractère" select="substring(translate(translate(Lemme/ReprésentationDeForme, '_^-‐‑*=', ''), $majuscules, $minuscules), 1, 1)"/>-->
            <!--<xsl:if test="$caractère != substring(translate(translate(preceding-sibling::EntréeLexicale[1]/Lemme/ReprésentationDeForme, '_^-‐‑*=', ''), $majuscules, $minuscules), 1, 1)">-->
                <!--<xsl:text>\newpage</xsl:text>-->
                <!--<xsl:text>\lettrine{</xsl:text>-->
    ...
    <xsl:template match="Sens">
        ...
        <xsl:apply-templates select="../Sens/Note"/>
    ...
    <xsl:template match="DomaineSémantique">
        ...
        <xsl:if test="not(following-sibling::DomaineSémantique)">
            <xsl:text>})</xsl:text>
        </xsl:if>
    </xsl:template>
